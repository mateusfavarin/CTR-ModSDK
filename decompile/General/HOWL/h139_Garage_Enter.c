  ...
  if (charId < 8)
  {
	...
	for (i = 0; i < 8; i++)
	{
      ...
      if (i == charId)
	  {
		...
		if (soundIDs[i] != 0) {
		  DECOMP_OtherFX_RecycleNew(audioPtr,(u_int)soundIDs[i],(int)garageSounds->volume << 0x10 | 0x8080);
		  continue;
		}
	  }
	  ...
	  else if (i == charLeft)
	  {
		...
		if (soundIDs[i] != 0) {
          DECOMP_OtherFX_RecycleNew(audioPtr,(u_int)soundIDs[i],(int)garageSounds->volume << 0x10 | 0x803c);
          continue;
        }
      }
      ...
      else if (i == charRight)
	  {
        ...
        garageSounds->LR = 0xc3;
	  	...
	  	if (soundIDs[i] != 0) {
          DECOMP_OtherFX_RecycleNew(audioPtr,(u_int)soundIDs[i],(int)garageSounds->volume << 0x10 | 0x80c3);
          continue;
        }
      }
