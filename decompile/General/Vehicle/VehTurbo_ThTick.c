...
void DECOMP_OtherFX_RecycleNew(void*, int, int);
...
void DECOMP_VehTurbo_ThTick(struct Thread* turboThread)
{
	...
	{
		// gamepad vibration
		DECOMP_GAMEPAD_ShockFreq(driver, 4, 4);
	}
	...
	if (instanceDriver->thread->modelIndex == 0x18)
	{
		...
		// driver audio
		DECOMP_OtherFX_RecycleNew(&driver->driverAudioPtrs[3], 0xe, (iVar7 << 0x10 | fireAudioDistort | 0x80));
	...
	else
	{
		...
		if (instanceDriver->thread->modelIndex == 0x18)
		{
			...
			// driver audio
			DECOMP_OtherFX_RecycleNew(&driver->driverAudioPtrs[3], 0xffffffff, uVar8);
		}
